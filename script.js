document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        es: {
            ...
            // Churn Intelligence
            'churn.title':        'Churn Intelligence System v3',
            'churn.desc':         'Sistema end-to-end que predice churn de clientes Telco, explica cada predicción con SHAP, optimiza presupuesto de retención por ROI y expone todo a través de un agente conversacional ReAct con Gemini. El mejor modelo (CatBoost, 54 features) alcanza ROC-AUC 0.847 en 10 ejecuciones.',
            'churn.li1.strong':   'ML Pipeline:',
            'churn.li1.text':     '4 modelos (RF, XGBoost, CatBoost, MLP) × 3 feature sets × 10 seeds con threshold óptimo por F1-OOF; tracking completo en MLflow.',
            'churn.li2.strong':   'Explicabilidad:',
            'churn.li2.text':     'SHAP TreeExplainer con waterfall plots por cliente; selección de features via Mutual Information y Hill Climbing (10 features → 98% del AUC).',
            'churn.li3.strong':   'Policy Engine:',
            'churn.li3.text':     'Optimizador ROI con 3 estrategias de retención (descuento, llamada, upgrade); asignación greedy de presupuesto por ROI esperado.',
            'churn.li4.strong':   'Serving:',
            'churn.li4.text':     'Agente ReAct (LangGraph + Gemini) con 5 tools; FastAPI REST + Streamlit UI; Evidently AI para drift monitoring y retrain automático.',
            // RAG
        ...
        en: {
            ...
            // Churn Intelligence
            'churn.title':        'Churn Intelligence System v3',
            'churn.desc':         'End-to-end system that predicts Telco customer churn, explains each prediction with SHAP, optimizes retention budget by ROI, and exposes everything through a conversational ReAct agent powered by Gemini. Best model (CatBoost, 54 features) reaches ROC-AUC 0.847 over 10 runs.',
            'churn.li1.strong':   'ML Pipeline:',
            'churn.li1.text':     '4 models (RF, XGBoost, CatBoost, MLP) × 3 feature sets × 10 seeds with optimal threshold by F1-OOF; full experiment tracking in MLflow.',
            'churn.li2.strong':   'Explainability:',
            'churn.li2.text':     'SHAP TreeExplainer with per-customer waterfall plots; feature selection via Mutual Information and Hill Climbing (10 features → 98% of AUC).',
            'churn.li3.strong':   'Policy Engine:',
            'churn.li3.text':     'ROI optimizer with 3 retention strategies (discount, call, upgrade); greedy budget allocation ranked by expected ROI.',
            'churn.li4.strong':   'Serving:',
            'churn.li4.text':     'ReAct Agent (LangGraph + Gemini) with 5 tools; FastAPI REST + Streamlit UI; Evidently AI for drift monitoring and auto-retrain trigger.',
            // RAG
